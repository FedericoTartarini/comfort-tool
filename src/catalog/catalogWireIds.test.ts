...
import { physicalQuantityWireIds } from "./quantities";
...
describe("catalog wire IDs", () => {
  it("keeps eslint restricted wire literals aligned with PhysicalQuantityId", () => {
    const eslintWireIds = parseEslintRestrictedWireStrings();
    expect(eslintWireIds).toEqual([...physicalQuantityWireIds]);
  });
});

...
describe("workspace navigation coordination", () => {
  it("preserves an eligible model while enforcing the route workspace", () => {
    ...
    expect(toolState.state.setting.activeSurface).toBe(SurfaceId.Standard);
  ...
  it("selects an ineligible route's default model without duplicating route lists", () => {
    ...
    expect(toolState.state.setting.selectedModel).toBe(ModelId.PmvIso);
    expect(toolState.state.setting.activeSurface).toBe(SurfaceId.Standard);
  ...
  it("navigates to the canonical model path when selecting a model", () => {
    ...
    expect(toolState.state.setting.selectedModel).toBe(ModelId.Utci);
  ...
  it("navigates to the canonical time-series model path", () => {
    ...
    expect(timeSeries.state.setting.selectedModel).toBe(ModelId.Phs2023);
    expect(navigate).toHaveBeenCalledTimes(1);
  ...
  it("lets pathname constraints win over a valid v1 share snapshot", () => {
    ...
    const ashraeSettings = source.state.setting.outputSettingsByModel[ModelId.PmvAshrae];
    ...
    expect(target.state.setting.selectedModel).toBe(ModelId.PmvAshrae);
    expect(target.state.setting.activeSurface).toBe(SurfaceId.Standard);
    expect(target.state.setting.outputSettingsByModel[ModelId.PmvAshrae].exploreBands?.[0].max)
      .toBe(-3.25);
    expect(target.actions.exportShareSnapshot().version).toBe(1);

    target.state.input.quantitiesByInput[InputId.Input1][PhysicalQuantityId.DryBulbTemperature] = 19;
    coordinator.prepareUrl(routeUrl("/standard/ashrae-55/?state=invalid"));
    coordinator.prepareUrl(sharedUrl, { validateRanges: false });
    expect(target.state.input.quantitiesByInput[InputId.Input1][PhysicalQuantityId.DryBulbTemperature])
  ...
  it("lets a path model slug win over a share snapshot selectedModel", () => {
    ...
    const coordinator = createWorkspaceNavigation(target, { navigate: vi.fn() });
    expect(coordinator.prepareUrl(sharedUrl, { validateRanges: false })).toBe(true);
    expect(target.state.setting.selectedModel).toBe(ModelId.HeatIndex);
    expect(target.state.setting.activeSurface).toBe(SurfaceId.Explore);
  });

  it("keeps SI input and model presentation state across the Time-series placeholder", () => {
    const toolState = createAnalysisState();
    const coordinator = createWorkspaceNavigation(toolState, { navigate: vi.fn() });
    toolState.state.input.quantitiesByInput[InputId.Input1][PhysicalQuantityId.DryBulbTemperature] = 21.5;
    toolState.state.setting.outputSettingsByModel[ModelId.PmvAshrae].xAxis =
      PhysicalQuantityId.MeanRadiantTemperature;
    const cacheBefore = toolState.state.output.calculationCacheByModel[ModelId.PmvAshrae];
    ...
    expect(toolState.state.input.quantitiesByInput[InputId.Input1][PhysicalQuantityId.DryBulbTemperature])
      .toBe(21.5);
    expect(toolState.state.setting.outputSettingsByModel[ModelId.PmvAshrae].xAxis)

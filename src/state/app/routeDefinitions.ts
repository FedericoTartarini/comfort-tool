...
export const standardRouteDefinitions = appRouteDefinitions.filter(
  (definition) => definition.surface === SurfaceId.Standard,
);
...
function getDefinitionBySurface(
  surface: SurfaceIdType,
): AppRouteDefinition | undefined {
  return appRouteDefinitions.find(
    (definition: AppRouteDefinition) =>
      definition.surface === surface && !definition.standardId,
  );
}
...
export function parseAppLocation(pathname: string): ParsedAppLocation | undefined {
  ...
  const surface = asSurfaceId(segments[0]);
  if (!surface) {
    return undefined;
  }

  if (surface === SurfaceId.Standard) {
    return parseStandardLocation(segments);
  }

  const definition = getDefinitionBySurface(surface);
  if (!definition) {
    return undefined;
  }
...
export function isCalculationRoute(
  ...
  surface: typeof SurfaceId.Standard | typeof SurfaceId.Explore;
  defaultModelId: ModelIdType;
} {
  return Boolean(
    definition
    && definition.defaultModelId
    && (definition.surface === SurfaceId.Standard || definition.surface === SurfaceId.Explore),
  );
}
...
export function isTimeSeriesRoute(
  ...
  surface: typeof SurfaceId.TimeSeries;
  defaultModelId: ModelIdType;
} {
  return Boolean(
    definition
    && definition.defaultModelId
    && definition.surface === SurfaceId.TimeSeries
  );
}

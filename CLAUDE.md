...
**Calculation ownership:** Model-specific thermal-comfort logic belongs in `src/declarations/**`. Reusable ChartType geometry belongs in `src/charts/`. Remaining shared psychrometric helpers, stress-band derivation, field-chart scaffolding, adapters, reference values, and cross-model utilities still live in `src/engines/comfort/**` until that layer moves. Psychrometric isoline geometry lives in `src/charts/psychrometric/` (T roots at ~0.001°C width); PMV declarations pass `evaluate(T, RH)` and band thresholds and do not solve isoline roots. TemperatureMode Air keeps `tr` from input on the Psychrometric field; Operative uses `tr=tdb` per sample and labels x Operative temperature. State and components must not contain raw formula implementations. Do not add a parallel geometry module under `engines/` for a ChartType that already has a `src/charts/` folder.
...
Use constants from `src/catalog/` for `ModelId` model identifiers (`src/catalog/modelIds.ts`), `PhysicalQuantityId` / `ChartAxisQuantityId` values, `ChartType` values (`src/catalog/chartTypes.ts`), `TableType` values (`src/catalog/tableTypes.ts`), compare-input identifiers, and `ZoneToken` values. `defineModel` charts are data-only Dynamic. Family modules use `FrontendChartDeclaration` with `type: ChartType`. Dropdown labels are `chartTypeLabel[type]`. Chart ids live on each declaration’s `charts` entries (`id`); the builder maps them to runtime `instanceId`. Figure assemble in `src/charts/` takes generic arrays and must not import models or quantities. Psychrometric humidity/isoline helpers live in `src/charts/psychrometric/` and take evaluate callbacks, not model or quantity ids. Do not introduce new raw domain strings for those concepts.
...
and dynamic chart wiring (isoline geometry in `src/charts/psychrometric/`), `heatLossSeries.ts` / `setSeries.ts`

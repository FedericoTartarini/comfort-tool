...
- **This file** describes **current** code. When a Plan slice deletes presets, the parallel `ChartInstanceId` tree, or `spec: unknown`, the Plan wins. Do not restore application-layer `*Dto` types or `src/models/comfortDtos.ts` to match older sentences here.
...
Use constants from `src/models/` for `ModelId` model identifiers (`src/models/modelIds.ts`), `PhysicalQuantityId` / `ChartAxisQuantityId` values, `ChartEngine` values (`src/models/chartEngines.ts`), `TableType` values (`src/models/tableTypes.ts`), compare-input identifiers, and `ZoneToken` values. `defineModel` charts are `ModelChartDeclaration`: a data-only union discriminated on `engine:` over existing engines (`DynamicField`, `BoundaryRegion`, `ParametricLine`, `BandScalar`, `TimeSeriesLine`). Optional `type` names an extended chart type on that same engine. Family modules use `FrontendChartDeclaration`. Do not use `spec: unknown`. Chart ids live on each declaration’s `charts` entries (`id`); the builder maps them to runtime `instanceId`. Do not recreate a parallel `ChartInstanceId` tree. `ParametricLine` interchange is polylines and optional limit bands. Do not introduce new raw domain strings for those concepts.
...
and dynamic chart construction, `heatLossSeries.ts` / `setSeries.ts`

...
- **This file** describes **current** code. When a Plan slice deletes presets, the parallel `ChartInstanceId` tree, `spec: unknown`, or Plotly-shaped `*Dto` types in `comfortDtos.ts`, the Plan wins. Do not restore application-layer `*Dto` request or chart-source types to match older sentences here.
...
and `pmvShared.ts` only for cross-standard

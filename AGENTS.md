...
- Thin models build `toLibrary` / `fromLibrary` from `inputs` + `response.values` (`mappingFromBinds` / `invokeMappedLibrary`). Family modules may still use `defineLibraryQuantityMapping()` for non-positional library calls and set `features.invoke` (plus `mapChartInput` / `buildChartSource` when the chart payload is not the SI bag). Aggregates and classifiers stay outside the map and outside `QuantityState`.

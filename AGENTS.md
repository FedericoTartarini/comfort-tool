...
- A model declaration may select Extra catalog ids via `extraQuantities` (PHS uses `phsPersonQuantityIds`). Extra quantities **must not** enter `primaryInputOrder` or the global share primary record; they live in sparse `modelInputsByModel`. PHS weight/height stay off the point-session input panel.
...
Stable IDs for those concepts come from `src/catalog/` only. Chart instance ids are derived from declaration `charts[].id`.
...
Share snapshots store `selectedChartInstanceId` per model. Instance ids are derived from declarations only.
...
- Authoring a model: [docs/adding-a-model.md](docs/adding-a-model.md). Copy `heatIndex.ts`, add a `ModelId` member, register once. Hard stops: new ChartType, new primary, new modifier, new Time-series session, new SI unit dimension. ChartType isoline roots are solved in `src/charts/`, not in a declaration.
